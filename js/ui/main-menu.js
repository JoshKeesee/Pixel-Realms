const mainMenu = {
  ...
  create(n) {
    ...
    this.gameIcon.onload = () =>
      setTimeout(() => (mainMenu.mainMenu.style.opacity = 1), 100);
  ...
  switchButton(val, set, scroll = true) {
    ...
    for (let i = 0; i < this.buttons; i++)
      document.querySelector(selector + i).style = "";
    document.querySelector(selector + this.curr).style.transform =
      "translateY(-2px)";
    if (mainMenu.r && this.curr > 2 && scroll)
      document
        .querySelector(selector + this.curr)
    ...
    if (mainMenu.r && this.curr != 0)
      return (document.querySelector(selector + this.curr).style.background = d
        ? "rgb(250, 0, 0)"
        : "rgb(0, 200, 0)");
    document.querySelector(selector + this.curr).style.background =
      "rgba(0, 0, 100, 0.8)";
    document.querySelector(selector + this.curr).style.borderColor =
      "rgb(0, 0, 255)";
    document.querySelector(selector + this.curr).style.boxShadow =
      "0 4px 10px rgb(0, 0, 255)";
  },
  click() {
    document.querySelector("#main-menu .button-" + mainMenu.curr).click();
    if (!this.r && this.curr != 3 && this.curr != 2)
      this.buttons = this.curr = 0;
  },
  ...
  async joinRoom(id) {
    ...
    if (!rooms[id])
      return mainMenu.notify("Couldn't find a room with that id.", "red", 5000);
    if (banned == id)
      return mainMenu.notify("You are banned from this room.", "red", 5000);
    if (getUser()) {
  ...
  async deleteRoom(id) {
    ...
    if (getUser()) {
      ...
      return mainMenu.notify(
        "You must be logged in to delete a room.",
        "red",
        5000,
      );
  ...
  async loadRooms() {
    ...
    mainMenu.buttons = 0;
    mainMenu.container.appendChild(
      mainMenu.createButton("Back", "back", () => mainMenu.backToMainMenu()),
    );
    ...
    mainMenu.privateJoin = mainMenu.createButton("Join", "join-room", () =>
      mainMenu.joinRoom(mainMenu.privateInput.value),
    );
    mainMenu.privateRoom.appendChild(mainMenu.privateInput);
    mainMenu.privateRoom.appendChild(mainMenu.privateJoin);
    const myRooms = [];
    Object.keys(rooms).forEach((k) =>
      rooms[k].creator == user.name ? myRooms.push(k) : "",
    );
    if (typeof user.id == "number")
      mainMenu.newRoom = mainMenu.createButton(
        "Create Room (" + myRooms.length + "/3)",
        "join-room",
        async () => {
          if (myRooms.length >= 3)
            return mainMenu.notify("You can't exceed the max limit for rooms.");
          ui.createPopup();
          ...
          const defaultMap = await ui.confirm(
            "Do you want to use the default map?",
          );
          const public = await ui.confirm("Is this room public?");
          const teamMap = await ui.confirm(
            "Do you want this room to have teams (PVP)?",
          );
          let numTeams = 0;
          if (teamMap)
            numTeams = Math.max(
              Math.min(
                await ui.prompt("How many teams do you want (Max 4)?"),
                4,
              ),
              2,
            );
    ...
    if (typeof user.id == "number")
      mainMenu.roomsContainer.appendChild(mainMenu.newRoom);
    mainMenu.roomList.innerHTML = "";
    Object.keys(rooms)
      .sort((a, b) => rooms[b].online - rooms[a].online)
      .forEach((k) => {
        if (
          !rooms[k].public &&
          rooms[k].creator != user.name &&
          !devs[user.name]
        )
          return;
        ...
        info.appendChild(id);
        r.appendChild(info);
        const join = mainMenu.createButton("Join", "join-room", () =>
          mainMenu.joinRoom(rooms[k].id),
        );
        r.appendChild(join);
        if (rooms[k].creator == user.name || devs[user.name])
          r.appendChild(
			...
			`,
              "delete-room",
              () => mainMenu.deleteRoom(rooms[k].id),
            ),
          );
        mainMenu.roomList.appendChild(r);
      });
    if (Object.keys(rooms).length == 0)
      mainMenu.roomList.innerText = "No rooms found...";
  },
  async showRooms() {
    mainMenu.r = true;
  ...
  async play(o = false, room) {
    ...
    mainMenu.buttons = 0;
    setTimeout(
      () => document.querySelector("#" + mainMenu.mainMenu.id).remove(),
      500,
    );
    nextTrack();
    ui.create();
  ...
  backToMainMenu() {
    ...
    mainMenu.buttons = 0;
    mainMenu.sideContainer.innerHTML = "";
    mainMenu.sideContainer.appendChild(
      mainMenu.createButton(
        "Singleplayer",
        "play",
        async () => await mainMenu.play(),
      ),
    );
    mainMenu.sideContainer.appendChild(
      mainMenu.createButton("Multiplayer", "play", () => mainMenu.showRooms()),
    );
    if (!getUser())
      mainMenu.sideContainer.appendChild(
        mainMenu.createButton("Login / Create Account", "play", () =>
          window.open(
            "login.html?server=" + serverUrl,
            "sub",
            `top=${(screen.height - 800) / 4},left=${(screen.width - 600) / 2},scrollbars=no,resizable=no,status=no,location=no,toolbar=no,menubar=no,frame=no,width=600,height=800`,
          ),
        ),
      );
    else
      mainMenu.sideContainer.appendChild(
        mainMenu.createButton("Logout", "play", () => {
          logoutUser();
          mainMenu.backToMainMenu();
        }),
      );
    ...
    setTimeout(
      () => {
        ...
        mainMenu.sideContainer.insertBefore(
          mainMenu.gameTitle,
          mainMenu.sideContainer.firstChild,
        );
      },
      mainMenu.r ? 500 : 0,
    );
  ...
  getRooms() {
    ...
    return new Promise((res) =>
      socket.emit("get rooms", (rooms) => res(rooms)),
    );
  },
};
